...
public class WhatsappRepository {
    ...
    public String changeAdmin(User approver, User user, Group group) throws Exception{
        ...
        if(!this.ISuserInGroup(group, user)) throw  new Exception("User is not a participant");
    ...
    public Group createGroup(List<User> users) {
        if(users.size() == 2) return this.createprivateChat(users);
    ...
    public Group createprivateChat(List<User> users) {
    ...
    public int sendMessage(Message message, User sender, Group group) throws Exception{
        if(!groupUserMap.containsKey(group)) throw new Exception("Group does not exist");
        if(!this.ISuserInGroup(group, sender)) throw  new Exception("You are not allowed to send message");
    ...
    public boolean ISuserInGroup(Group group, User sender) {
